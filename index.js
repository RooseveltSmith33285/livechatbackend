...
export default async function handler(req, res) {
  ...
  try {
    ...
    // console.log('batchusers:', batchUsers.length);

    // // Send email notification if count is low
    // if (count <= 15) {
    //   const mailOptions = {
    ...
    //     service: 'gmail',
    //     auth: {
    //       user: process.env.GMAIL_USER || 'leads@enrichifydata.com', 
    //       pass: process.env.GMAIL_PASS || 'cazhzgbslrzvyjfc' 
    //     }
    //   });

    //   await transporter.sendMail(mailOptions);
    //   console.log("Low inventory email sent successfully");
    // }

    // console.log("Processing", uniqueUnenrichedLimited.length, "leads for enrichment");
    
    // // Call your enrichment function
    // await enrichFile(uniqueUnenrichedLimited);
    ...
    //     processedForEnrichment: uniqueUnenrichedLimited.length,
    //     lowInventoryEmailSent: count <= 15
    //   },
    ...
    res.status(500).json({ 
      success: false,
      error: 'Lead enrichment cron job failed',
